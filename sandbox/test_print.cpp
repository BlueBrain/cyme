...
// typedef boost::mpl::vector< vector_f_aos, vector_f_aosoa, vector_d_aos,
// vector_d_aosoa > vector_list;
...
void print(boost::accumulators::accumulator_set<double, stats<tag::mean, tag::variance>> const &acc, std::string name) {
    ...
    std::cout << "bench: " << name << " " << extract_result<tag::mean>(acc) << " [s], variance "
              << extract_result<tag::variance>(acc) << ", standard deviation "
              << sqrt(extract_result<tag::variance>(acc)) << std::endl;
}
...
struct Na {
    ...
    /*
         ...
         static inline  cyme::serial<value_type,O> cnrn_current(iterator it,
     value_type t = value_type()){
         ...
         static inline void cnrn_initmodel(iterator it){
             (*it)[4] = 0. ; // TO DO, (*it)[4] ^= (*it)[4] XOR, or basic set up
     to 0 <--------------------------------- TIM
             (*it)[3] = 0. ;
             cnrn_rates(it);
             (*it)[3] = (*it)[6];
         ...
         static inline void cnrn_cur(iterator it){
             ...
             cyme::serial<value_type,O> tmp  = cnrn_current<iterator, O>(it, _v +
     0.001);
...
template <class mechanism, class container = cyme::vector<mechanism, cyme::AoSoA>> // should use
// cyme
// container
// there
class pack {
  public:
    ...
    value_type &operator()(size_t i, size_t j) { return cont(i, j); }

    std::size_t size() { return cont.size(); }

    std::size_t size_block() { return cont.size_block(); }

...
namespace cyme{
    ...
    /** Helper for polynom where 0 <= x <= Pi/4 (cos)*/
    ...
    /** Helper for polynom where Pi/4 <= x <= Pi/2 (sin)*/ 
    template<class T, cyme::simd O, int N>
    ...
    /** Helper for polynom (tan)*/ 
    template<class T, cyme::simd O, int N>
    struct Poly_helper<T,O,N,coeff_cephes_tan>{
	static forceinline vec_simd<T,O,N> poly(vec_simd<T,O,N> x){
	    vec_simd<T,O,N> z(x*x);
	    vec_simd<T,O,N> y = helper_horner<T,O,N,coeff_cephes_tan,
					      poly_order<T,coeff_cephes_tan>::value>::horner(z);
	    y *= x;
	    y += x;
	    return y;
	}
    ...
    */
	...
	const vec_simd<T,O,N> PIO2(1.5707963267948966192313216916397514420985846996875529);
	return sin(rhs + PIO2);
    }
	...
	//Select Polynomial
 	vec_simd<T,O,N> poly = Poly_helper<T,O,N,coeff_cephes_tan>::poly(z);
	//poly = select_tan(poly,x);
	return poly;//select_sign(j,TWO,rhs,poly);
    }

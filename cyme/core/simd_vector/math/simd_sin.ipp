...
namespace cyme{
    ...
    /** Base struct*/
    template<class T, cyme::simd O, int N, std::size_t n = poly_order<T,coeff_sin_cos::value>, int p>
    struct Poly_helper;

   
    /** Helper for polynom where 0 <= x <= Pi/4 (sin_lo)*/
    template<class T, cyme::simd O, int N, std::size_t n = poly_order<T,coeff_sin_cos::value> >
    struct Poly_helper<T,O,N,n,0>{
    ...
    template<class T, cyme::simd O, int N, std::size_t n = poly_order<T,coeff_sin_cos>::value,
             int p, class Solver = Poly_helper<T,O,N,n,p> > //sin_hi or sin_lo

...
BOOST_AUTO_TEST_CASE_TEMPLATE(block_init_default_constructor, T, floating_point_block_types) {
     memory::block_a<TYPE,M,N,ORDER> block_a;
     TYPE test[M*N];
...
BOOST_AUTO_TEST_CASE_TEMPLATE(block_init_value_constructor, T, floating_point_block_types) {
     ...
     memory::block_a<TYPE,M,N,ORDER> block_a(random);
     TYPE test[M*N];
...
BOOST_AUTO_TEST_CASE_TEMPLATE(block_operator_equal_multiple, T, floating_point_block_types) {
    ...
    memory::block_a<TYPE,4,N,memory::AoSoA> block_b;

    init(block_a, block_b);

    typename memory::block_a<TYPE,4,N,memory::AoS>::iterator it_AoS = block_a.begin();
    for(; it_AoS != block_a.end(); ++it_AoS){
        (*it_AoS)[0] = (*it_AoS)[0];
        (*it_AoS)[0] = (*it_AoS)[1];
        (*it_AoS)[0] = (*it_AoS)[2];
    ...
    typename memory::block_a<TYPE,4,N,memory::AoSoA>::iterator it_AoSoA = block_b.begin();
    for(; it_AoSoA != block_b.end(); ++it_AoSoA){

...
//           return ((a > -limit) && (a < limit)) ? Solver::exp(a) : std::exp(a);
    ...
    */
    template<class T, std::size_t n>
    inline void exp(T* a, T const* b){
   //     vec_simd<T,memory::__GETSIMD__()> v(b); // init register one cycle
   //     vec_simd<T,memory::__GETSIMD__()> nrv = exp<numeric::vec_simd<T,memory::__GETSIMD__()>,n>(v); // copy register one cycle
